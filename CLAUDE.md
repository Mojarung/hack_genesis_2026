...
- Ruby 4.0.6 + DevKit стоит в `C:\Ruby40-x64`. В системном PATH лежит **Ruby 3.4**, поэтому
  каждую сессию первой строкой: `$env:PATH = "C:\Ruby40-x64\bin;" + $env:PATH` — без неё
  `bundle` возьмёт 3.4 и упадёт на `Bundler::RubyVersionMismatch`.
...
| Эталон | `lib/payout_router/analytics/assignment_bound.rb` | оптимальное распределение очереди как транспортная задача (`MinCostFlow`): наш роутинг против всезнающего оптимума |
...
235 спеков зелёные (покрытие строк 96.3%, ветви 79.9%), rubocop чист, чистая копия в Docker `ruby:4.0` проходит всё (с YJIT ≈3 300 заявок/с), `rake stress` — 17 сценариев и ~16 000 заявок
с нулём нарушений инвариантов, бэктест conversion_first +4.7% / balanced −5.0% (цена удержания долей),
бенчмарк ≈10 600 заявок/с на Ryzen 7 9700X и ≈2 700 на ноутбуке разработки (оба без YJIT), деградации на объёме нет: 200 000 заявок идут с той же скоростью, что 5 000. Разбор QA-сессии и чекпоинта 1 (оба 04.09) —
