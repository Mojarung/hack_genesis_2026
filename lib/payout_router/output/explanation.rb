...
module PayoutRouter
  module Output
    ...
    class Explanation
      def initialize(decision, verbose: false)
        @decision = decision
        @verbose = verbose
      end
      ...
      def lines
        ...
        [header, *@decision.attempts.flat_map { |attempt| attempt_lines(attempt) }, "", outcome_line]
      end
      ...
      def attempt_lines(attempt)
        ...
        if attempt.breakdown && (attempt.selected? || attempt.dispatched? || @verbose)
          lines.concat(breakdown_lines(attempt.breakdown))
        end
        lines
      end
