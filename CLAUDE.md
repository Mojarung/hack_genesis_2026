...
| Эталон | `lib/payout_router/analytics/assignment_bound.rb` | оптимальное распределение очереди как транспортная задача (`MinCostFlow`), CLI `bound`: наш роутинг против всезнающего оптимума. Self-provider доступен эталону ровно в том объёме, в каком его использовал роутер, — иначе задача вырождается в «всё себе». Считается **в каждом прогоне** и лежит в отчёте блоком `optimality` (`Runner#route`); главная цифра `score_vs_optimum` — доля от `free_optimum`, единственной настоящей верхней границы. От `quota_optimum` её считать нельзя: доли у роутера мягкая цель, и на длинной очереди он его законно обходит |
...
254 спека зелёные (покрытие строк 96.2%, ветви 80.3%), rubocop чист, чистая копия в Docker `ruby:4.0` проходит всё (спеки, rubocop, оба валидатора, стресс — проверено 06.09), `rake stress` — 17 сценариев и ~16 000 заявок
с нулём нарушений инвариантов, бэктест conversion_first +4.7% / balanced −5.0% (цена удержания долей),
бенчмарк (замер 06.09 в Docker `ruby:4.0` с YJIT) 9 942 заявки/с на 50 тыс. и 9 399 на 200 тыс.; на этом ноутбуке без YJIT 5 445–5 827 на тех же объёмах — деградации на объёме нет. Разбор QA-сессии и чекпоинта 1 (оба 04.09) —
...
и одобрения. Чекпоинт 3 (06.09, раздел 4.7) добавил немного: провайдеры к тестовой очереди
**не меняются** (в 22:00 подкладываем только очередь), сдача через GitHub, презентация свободного
формата, критерий по коду — структурность и «не всё в одном файле». Открытые вопросы к жюри —
пять с половиной, раздел 5: резервировать ли дневной лимит под in-progress, показывать ли отказы
в сдаваемом файле, как проверяют отчёт, что считается «поведением при невыполнимой цели»,
как мерить успешность, каков размер тестовой очереди. **Про дневной лимит эксперты вердикта
не давали** — «считаем по выполненным заявкам» это наша реплика на чекпоинте, а не их одобрение.
