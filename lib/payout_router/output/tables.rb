...
module PayoutRouter
  module Output
    ...
    module Tables
      ...
      # Эталонное распределение: где мы между худшим допустимым назначением и оптимумом.
      def bound(result)
        [
          %w[назначение одобрений комментарий],
          ["оптимум", result.free_optimum.round(2), "знает всю очередь заранее, без ёмкостных лимитов"],
          ["наш роутинг", result.ours.round(2),
           "онлайн, по одной заявке; self-provider использован #{result.self_budget} раз"],
          ["худшее допустимое", result.worst.round(2), "нижняя точка отсчёта при той же квоте self"],
          ["— оптимум, если ещё и держать доли", result.quota_optimum.round(2),
           "другая задача: никто из внешних не берёт больше своей целевой доли"],
          ["— вся очередь на self-provider", result.all_to_self.round(2),
           "вырожденный максимум: нет истории, заявленные 0.95, нет лимитов"]
        ]
      end
